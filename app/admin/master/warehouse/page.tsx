...
import FormWarehouse from "@/components/form-modal/admin/master/warehouse-form";
...
export default function WarehousePage() {
  ...
  const { data, isLoading, refetch } = useGetWarehouseListQuery({
    page: currentPage,
    paginate: itemsPerPage,
    search: query,
  });

  const warehouseList = useMemo(() => data?.data || [], [data]);
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingId) {
        await updateWarehouse({ id: editingId, payload }).unwrap();
        Swal.fire("Sukses", "Warehouse diperbarui", "success");
      } else {
        await createWarehouse(payload).unwrap();
        Swal.fire("Sukses", "Warehouse ditambahkan", "success");
      }
  ...
  const handleDetail = (item: Warehouse) => {
    setForm(item);
    setReadonly(true);
    openModal();
  };
  ...
  const handleDelete = async (item: Warehouse) => {
    ...
    if (confirm.isConfirmed) {
      try {
        await deleteWarehouse(item.id).unwrap();
        await refetch();
        Swal.fire("Berhasil", "Warehouse dihapus", "success");
      } catch (error) {
      ...
      <div className="rounded-md bg-white p-4 border border-gray-100 shadow-sm">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="shrink-0 flex flex-wrap items-center gap-2">
            ...
            {openModal && <Button onClick={openModal}><Plus /> Warehouse</Button>}
          </div>
        </div>
      </div>
