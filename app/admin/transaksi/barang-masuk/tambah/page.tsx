...
import { BarangMasukHeader } from "../page"; // Impor dari file list
...
const useAddBarangMasukMutation = () => {
...
export default function TambahBarangMasukPage() {
  ...
  const [header, setHeader] = useState<Partial<BarangMasukHeader>>({
    tanggal: new Date().toISOString().split('T')[0], // Default hari ini
    keterangan: "",
    kode_transaksi: "", // Sebaiknya di-generate server
  });
  const [details, setDetails] = useState<BarangMasukDetail[]>([]);

  // State untuk form 'Add Item'
  const [currentItem, setCurrentItem] = useState<Partial<BarangMasukDetail>>({
    jumlah: 1,
  });

  // Hook mutasi untuk menyimpan
  const { mutate: addTransaksi, isLoading } = useAddBarangMasukMutation();
  ...
  const handleAddItem = () => {
    ...
    if (existingItemIndex > -1) {
      ...
      setDetails(prev => [
        ...prev,
        {
          ...currentItem as BarangMasukDetail, // Asumsikan sudah tervalidasi
        }
      ]);
    }
  ...
  const handleSimpanTransaksi = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    addTransaksi(dataToSubmit, {
      onSuccess: () => {
        ...
        router.push("/admin/transaksi/barang-masuk"); // Kembali ke halaman list
      },
      onError: () => {
