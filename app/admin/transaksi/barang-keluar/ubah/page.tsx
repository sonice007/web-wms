...
import { CreateBarangKeluarRequest, CreateBarangKeluarDetailRequest } from "@/types/admin/transaksi/barang-keluar";
...
interface BarangKeluarDetailLokal extends CreateBarangKeluarDetailRequest {
  barang_name?: string;
...
// Terkeluar 'warehouse_id' sementara untuk filter rak
interface CurrentItemState extends Partial<BarangKeluarDetailLokal> {
...
export default function TambahBarangKeluarPage() {
  ...
  // State untuk Header
  const [header, setHeader] = useState<Partial<CreateBarangKeluarRequest>>({
  ...
  const [details, setDetails] = useState<BarangKeluarDetailLokal[]>([]);
  ...
  const [createBarangKeluar, { isLoading: isCreating }] = useCreateBarangKeluarMutation();
  ...
  const handleAddItem = () => {
    ...
    const itemToAdd: BarangKeluarDetailLokal = {
  ...
  const handleSimpanTransaksi = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const detailsToSubmit: CreateBarangKeluarDetailRequest[] = details.map(d => ({
      ...
      const payload: CreateBarangKeluarRequest = {
        warehouse_id: header.warehouse_id,
        date: header.date!,
        partner: header.partner || null,
        notes: header.notes || null,
        details: detailsToSubmit,
      };

      await createBarangKeluar(payload).unwrap();
      await Swal.fire("Sukses", "Barang ditambahkan", "success");
      router.push("/admin/transaksi/barang-keluar");
    } catch (error) {
      console.error(error);
  ...
  return (
    <form onSubmit={handleSimpanTransaksi} className="p-6 space-y-6">
      ...
      <Card>
        <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-6"> {/* Added pt-6 for padding */}
          ...
          <div className="space-y-2">
            ...
            <Input
              id="partner"
              type="text"
              placeholder="Nama Partner (misal: Toko XYZ)"
              value={header.partner ?? ""}
              onChange={(e) => setHeader(p => ({ ...p, partner: e.target.value }))}
            />
          </div>
          <div className="space-y-2">
          ...
          <div className="space-y-2 md:col-span-3">
            ...
            <Textarea
              id="keterangan"
              placeholder="Keterangan transaksi (misal: Penerimaan dari Partner X)"
              value={header.notes ?? ""}
