...
const run = async () => {
  ...
  // Prefer `wms.webp` else fallback to `wharehouse-management-system-logo.png` or others
  const candidates = ['wms.webp', 'wharehouse-management-system-logo.png', 'wms.webp'];
  const source = candidates.map((c) => path.join(publicDir, c)).find((p) => fs.existsSync(p));

  if (!source) {
    console.error('No source logo found. Put your logo in `public/wms.webp` or `public/wharehouse-management-system-logo.png`.');
    process.exit(1);
  }
