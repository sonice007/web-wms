...
import { 
  BarangMasuk, 
  // BarangMasukDetail, // Tipe ini sudah termasuk dalam BarangMasuk
  // BarangMasukApiResponse // Tipe ini ditangani oleh hook RTK Query
...
// (data: BarangMasuk) -> Asumsi tipe BarangMasuk Anda sudah benar
// sesuai API (memiliki 'warehouse: object' dan 'details: array')
const generatePDF = (data: BarangMasuk) => {
  const doc = new jsPDF();
  ...
  // Info Kiri (Supplier)
  doc.setFont("helvetica", "bold");
  doc.text("SUPPLIER:", pageMargin, cursorY);
  cursorY += 5;
  doc.setFont("helvetica", "normal");
  doc.text(data.supplier || "N/A", pageMargin, cursorY);
  ...
  doc.setFont("helvetica", "bold");
  doc.text("Tanggal PO:", rightColX, cursorY);
  doc.setFont("helvetica", "normal");
  doc.text(formatDate(data.date), rightColX + 35, cursorY); // +35 untuk alignment
  cursorY += 5;

  doc.setFont("helvetica", "bold");
  doc.text("Jatuh Tempo:", rightColX, cursorY);
  doc.setFont("helvetica", "normal");
  doc.text(formatDate(data.due_date), rightColX + 35, cursorY);
  cursorY += 5;
  ...
  // 7. Simpan file
  doc.save(`PO_${data.reference}.pdf`);
};

// Komponen Tombol
interface InvoicePOButtonProps {
  itemId: number;
}

export function InvoicePOButton({ itemId }: InvoicePOButtonProps) {
  // --- DIPERBAIKI ---
  // Menggunakan hook query yang asli dari RTK Query
  const { data, isLoading, refetch } = useGetBarangMasukByIdQuery(itemId);
